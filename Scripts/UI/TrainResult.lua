module("TrainResult", package.seeall)
...
function create()
	...
	local uiLayout = GUIReader:shareReader():widgetFromJsonFile(PATH_RES_UI .. "trainResult_1.json");
	...
	m_diamondLabel = tolua.cast(m_uiLayer:getWidgetByName("jinbi__labelNum"),"LabelAtlas");
	m_scoreLabel = tolua.cast(m_uiLayer:getWidgetByName("exp__labelNum"),"LabelAtlas");
	-- moneyLabel:setText(tostring(rewardCommon.money));

	m_diamond = rewardCommon.money;
	m_score = rewardCommon.exp;
